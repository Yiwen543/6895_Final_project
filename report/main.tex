...
\title{Cathey: An Offline Voice-Controlled Smart Home Assistant on Raspberry Pi 5 with a Four-Layer Memory and a Hybrid Rule-Based / Quantized LLM Pipeline}
...
Our project, called Cathey, is an attempt to answer that question. Cathey is an offline smart home assistant that listens for the wake word "Cathey", understands what the user said, and either controls a real device (an LED ring and a stepper-motor curtain in our prototype) or replies in natural speech. Everything happens on a single Raspberry Pi 5 with 8\,GB of RAM. Nothing is sent to the cloud.
...
\begin{itemize}
    ...
    \item \texttt{agent.py}: \texttt{CatheyAgent}, the dialogue state machine.
...
$\downarrow$ \\
CatheyAgent dispatcher (4 intent types) \\
$\downarrow$ \\
...
\begin{itemize}
    ...
    \item \textbf{Service management:} \texttt{systemd} with two services (\texttt{bt-speaker.service}, \texttt{cathey.service}).
\end{itemize}

\subsection{Dialogue State Machine}

\texttt{CatheyAgent} keeps a small state object with a flag \texttt{pending\_clarification}. The flow is:
\begin{enumerate}
...
\begin{enumerate}
    \item Step 1: ``Cathey, I feel cold.'' $\rightarrow$ Nova asks for clarification.
    ...
    \item Step 4: ``Cathey, I feel cold.'' $\rightarrow$ Nova does not ask; it executes ``close window'' directly because it remembers from step 2.
\end{enumerate}
...
We built Cathey, a fully offline voice-controlled smart home assistant for Raspberry Pi 5. The system combines Whisper STT, a 3B Qwen2.5 LLM in 3-bit GGUF quantization, a rule-based fast path, a four-layer memory, LoRA fine-tuning, and Piper TTS. Everything runs on-device, with no cloud calls.
